  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        // Append the selectedGuestId only if it's defined and valid
        if (selectedGuestId) {
    ...
    if (
      ...
      try {
        if (selectedGuestId) {
          ...
          const reservationCheckResponse = await axios.get(
            `${config.API_URL}/reservations/guest/${selectedGuestId}`
          );

          if (
            reservationCheckResponse.data.success &&
            reservationCheckResponse.data.reservations.length > 0
          ) {
            showSnackbar(
              "This guest already has an active reservation.",
              "warning"
            );
            return; // Stop further processing if the guest has a reservation
          }

          // Check if the guest already has a stay record
          const stayRecordCheckResponse = await axios.get(
            `${config.API_URL}/stay_records/guest/${selectedGuestId}`
          );

          if (
            stayRecordCheckResponse.data.success &&
            stayRecordCheckResponse.data.stay_records.length > 0
          ) {
            showSnackbar(
              "This guest already has an active Check-In.",
              "warning"
            );
            return; // Stop further processing if the guest has a stay record
          }
            ...
            {activeStep === 0 && (
              <Grid container spacing={2} sx={{ mt: 2 }}>
                <FormSectionArray>
                  ...
                  <Grid item xs={12}>
                    {/* Conditionally render Autocomplete component based on selectedGuestId */}
                    <Autocomplete
                      freeSolo
